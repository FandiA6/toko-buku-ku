...
if(@$data['id_buku']==" "){
  $id_buku = "BU00000000001";
}
else{
  @$id_buku = substr($data['id_buku'],2);
  @$id_buku = intval($id_buku);
  if($id_buku <9){
    $id_buku = "BU0000000000".( $id_buku+1);
  }
  elseif($id_buku <99){
    $id_buku = "BU000000000".( $id_buku+1);
  }
  elseif($id_buku <999){
    $id_buku = "BU00000000".( $id_buku+1);
  }
  elseif($id_buku <9999){
    $id_buku = "BU0000000".( $id_buku+1);
  }
  elseif($id_buku <99999){
    $id_buku = "BU000000".( $id_buku+1);
  }
  elseif($id_buku <999999){
    $id_buku = "BU00000".( $id_buku+1);
  }
  elseif($id_buku <9999999){
    $id_buku = "BU0000".( $id_buku+1);
  }
  elseif($id_buku <99999999){
    $id_buku = "BU000".( $id_buku+1);
  }
  elseif($id_buku <999999999){
    $id_buku = "BU000".( $id_buku+1);
  }
  elseif($id_buku <9999999999){
    $id_buku = "BU00".( $id_buku+1);
  }
  elseif($id_buku <99999999999){
    $id_buku = "BU0".( $id_buku+1);
  }
}
?>
         ...
         <div class="form-group">
		 			...
		 			<div class="col-md-offset-2">
		 				<?php 
		 					if (@$_GET['id']=="") {
		 				 ?>
		 				<button name="simpan" class="btn btn-primary active mt-3">Simpan</button>
		 				<?php }else{ ?>
		 				<button name="update" class="btn btn-primary active mt-3">Update</button>
		 				<?php } ?>
		 			</div>
		 		</div>
		 	</div>
    ...
    <tbody class="table-primary">
        <?php 
          ...
          <tr>
              ...
              <td><?=  $r['ppn'] ?></td>
              <td><?=  $r['diskon'] ?></td>
              <td><a class="btn btn-primary text-light hover active" href="?menu=buku&hapus&id=<?php echo $r['id_buku'] ?>" onclick="return confirm('yakin mau hapus?')">Hapus</a></td>
              <td><a class="btn btn-primary text-light hover active" href="?menu=buku&edit&id=<?php echo $r['id_buku'] ?>">Edit</a></td>
          </tr>
          <?php }  ?>
    </tbody>
